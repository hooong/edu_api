...
class AuthService:
    ...
    def signup(self, request: SignupRequest) -> int:
        if self.uow.user_repository.email_exists(email=request.email):
            raise ConflictError("이미 존재하는 이메일입니다.")

        user = User.create(email=request.email, password=request.password)
        self.uow.user_repository.save(user)

        return user.id

    def login(self, request: LoginRequest) -> str:
        user: User | None = self.uow.user_repository.get_by_email(request.email)
        if not user:
